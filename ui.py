...
class UIManager:
    ...
    def get_text(self, key, lang=None):
        ...
        return cfg.TEXT_DATA[lang].get(key, "")

    def update_all_texts(self):
        lang = self.current_lang
        data = cfg.TEXT_DATA[lang]
        ...
        self.app.lang_label.configure(text=data["lang_lbl"])
    ...
    def refresh_status(self):
        ...
        if "Удалено:" in current_status or "Deleted:" in current_status:
            ...
            txt = "Ошибка доступа" if lang == "ru" else "Access Error"
            self.app.status_label.configure(text=txt, text_color=cfg.COLOR_ERROR)
            return
    ...
    def refresh_log(self):
        ...
        for event_type, value in log_history:
            template = cfg.TEXT_DATA[lang].get(event_type, "{}")
            if "{}" in template:
...
class CleanupController:
    ...
    def start_cleanup(self):
        ...
        self.app.clean_button.configure(state="disabled", text="Очистка...")
    ...
    def _on_success(self, deleted, skipped, freed_mb):
        self.app.after(0, lambda: self.app.status_label.configure(
            text=cfg.TEXT_DATA[self.app.ui.current_lang]["status_success"].format(deleted, skipped, freed_mb),
            text_color=cfg.COLOR_SUCCESS
        ))

    def _on_error(self, err_type):
        self.app.after(0, lambda: self.app.status_label.configure(
            text="Ошибка" if self.app.ui.current_lang == "ru" else "Error",
...
def build_ui(app):
    ...
    app.lang_switch = ctk.CTkComboBox(
        app.settings_frame, width=80, font=cfg.FONT_NORMAL,
        corner_radius=6, values=["RU", "EN"], command=app.ui.change_language
    )
    app.lang_switch.pack(side="right", padx=25, pady=15)

    app.lang_label = ctk.CTkLabel(app.settings_frame, text="", font=cfg.FONT_NORMAL)
    app.lang_label.pack(side="right", padx=5, pady=15)
...
class WinClean64App(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"{cfg.APP_NAME} - {cfg.APP_VERSION}")
        self.geometry(f"{cfg.MAIN_WINDOW_WIDTH}x{cfg.MAIN_WINDOW_HEIGHT}")
        self.resizable(False, False)

        # Настройка цветов для customtkinter
        for i in range(100):
    ...
    def open_about_window(self):
        AboutWindow(self, cfg.TEXT_DATA[self.ui.current_lang], self.github_url, self.APP_NAME, self.APP_VERSION)
