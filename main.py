...
class WinClean64App(ctk.CTk):
    ...
    def update_interface_text(self):
        ...
        if "Удалено:" in current_status or "Deleted:" in current_status:
            ...
            self.status_label.configure(text=txt, text_color="#e74c3c")
        else:
            self.status_label.configure(text=data["status_idle"], text_color=("gray50", "gray70"))
    ...
    def start_clean_thread(self):
        ...
        # Запускаем логику из cleaner.py и передаем безопасные колбэки через .after()
        self.clean_thread = threading.Thread(
            target=clean_temp_files_backend,
            args=(
                lambda path: self.after(0, lambda: self.log_history.clear() or self.add_to_log("log_start", path)),
                lambda et, val: self.after(0, lambda: self.add_to_log(et, val)),
                lambda d, s, m: self.after(0, lambda: self.status_label.configure(
                    text=self.text_data[self.current_lang]["status_success"].format(d, s, m),
                    text_color=("#27ae60", "#2ecc71")
                )),
                lambda err_type: self.after(0, lambda: self.status_label.configure(
                    text="Ошибка" if self.current_lang == "ru" else "Error", 
                    text_color="#e74c3c"
                )),
                lambda: self.after(0, self._finish_cleanup)
            ),
            daemon=True
        )
